...
class ApicloudLoaderAndroidKeyCommand(sublime_plugin.TextCommand):
    ...
    def run(self, edit):
        logging.basicConfig(level=logging.DEBUG,
                            ...
                            datefmt='%Y %m %d  %H:%M:%S',
                            filename=os.path.join(curDir, 'mx_app_loader.log'),
                            filemode='a')
...
class ApicloudLoaderAndroidCommand(sublime_plugin.WindowCommand):
    ...
    def run(self, dirs):
        logging.basicConfig(level=logging.DEBUG,
                            ...
                            filename=os.path.join(
                                self.__curDir, 'mx_app_loader.log'),
                            filemode='a')
        sublime.status_message(u'开始真机同步')
    ...
    def checkBasicInfo(self):
        ...
        if 'darwin' in platform.system().lower():
            self.__adbExe = '"' + \
                os.path.join(self.__curDir, 'tools', 'adb') + '"'
        elif 'windows' in platform.system().lower():
            self.__adbExe = '"' + \
                os.path.join(self.__curDir, 'tools', 'adb.exe') + '"'
        else:
    ...
    def getLoaderType(self, appId):
        ...
        appIdPath = os.path.join(
            self.__curDir, 'appLoader', 'custom-loader', appId)
        logging.info('getLoaderType: appIdPath is ' +
                     os.path.join(appIdPath, 'load.conf'))
        ...
        if os.path.exists(os.path.join(appIdPath, 'load.conf')) and os.path.exists(os.path.join(appIdPath, 'load.apk')):
            ...
            with open(os.path.join(appIdPath, 'load.conf')) as f:
                config = json.load(f)
                logging.info(
                    'getLoaderType: load.conf content is ' + str(config))
                if 'version' in config:
                ...
                logging.info('getLoaderType: pendingVerion is ' +
                             self.__pendingVersion)
                logging.info('getLoaderType: pkgName is ' + self.__pkgName)
        else:
            self.__pkgName = 'com.minxing.client'
            self.__loaderName = 'apicloud-loader'
            logging.info(
                'getLoaderType: path not exiest, will use default appLoader')
        pass
    ...
    def pushDirOrFileCmd(self, serialNumber, srcPath, appId):
        ...
        logging.info('begin pushDirOrFileCmd from ' +
                     srcPath + ' for appId ' + appId)
        sublime.status_message(u'开始推送widget包')
        desPath = '/sdcard/minxing_dev/appstore/plugin/apps/debug/' + appId
        pushCmd = self.__adbExe + ' -s ' + serialNumber + \
            ' push "' + tmpPath + '" ' + desPath
    ...
    def compareAppLoaderVer(self, deviceVersion, appLoaderVersion):
        logging.info('begin compareAppLoaderVer ' +
                     deviceVersion + ' ' + appLoaderVersion)
    ...
    def getApploaderVersionCmd(self, serialNumber):
        ...
        version = -1
        cmd = self.__adbExe + ' -s ' + serialNumber + \
            ' shell dumpsys package ' + self.__pkgName
    ...
    def startApploaderCmd(self, serialNumber, appId):
        ...
        startCmd = self.__adbExe + ' -s ' + serialNumber + \
            ' shell am start -W -n ' + appLoaderPkg + ' --es "debugAppId" ' + appId
    ...
    def stopApploaderCmd(self, serialNumber):
        ...
        stopCmd = self.__adbExe + ' -s ' + serialNumber + \
            ' shell am force-stop ' + self.__pkgName
    ...
    def uninstallApploaderCmd(self, serialNumber):
        ...
        uninstallCmd = self.__adbExe + ' -s ' + \
            serialNumber + ' uninstall ' + self.__pkgName
        ...
        if 'Success' not in uninstallOutput:
            logging.info(
                'uninstallApploaderCmd: failed to run uninstallApploaderCmd!')
            return False
...
class ApicloudLoaderIosCommand(sublime_plugin.WindowCommand):
    ...
    def loadIos(self, srcPath):
        ...
        if 'windows' in platform.system().lower():
            ...
            (rtnCode, stdout, stderr) = runShellCommand(
                'java -version', self.__cmdLogType)
            outputMsg = stdout + stderr
        ...
        if 'windows' in platform.system().lower():
            javaCmd = os.path.join(
                self.__curDir, 'tools', 'jre', 'bin', 'java')
        else:
            javaCmd = 'java'
        ...
        iosLoaderPath = os.path.join(
            self.__curDir, 'appLoader', self.__loaderName)
    ...
    def getIosLoaderType(self, appId):
        ...
        appIdPath = os.path.join(
            self.__curDir, 'appLoader', 'custom-loader-ios', appId)
        logging.info('getIosLoaderType: appIdPath is ' +
                     os.path.join(appIdPath, 'load.conf'))
        ...
        if os.path.exists(os.path.join(appIdPath, 'load.conf')) and os.path.exists(os.path.join(appIdPath, 'load.ipa')):
            ...
            with open(os.path.join(appIdPath, 'load.conf')) as f:
                config = json.load(f)
                logging.info(
                    'getIosLoaderType: load.conf content is ' + str(config))
                if 'version' in config:
                    version = config['version'].strip()
                if 'packageName' in config:
                    pkgName = config['packageName'].strip()
            ...
            logging.info(
                'getIosLoaderType: path not exiest, will use default appLoader')
        pass
...
def BeforeSystemRequests():
    ...
    def get_system_version():
        ...
        if system_name == 'Windows' and os.name == 'nt':
            system_machine = platform.platform().split(
                '-')[0] + platform.platform().split('-')[1]
...
class NewApicloudDefaultAppCommand(sublime_plugin.WindowCommand):
    def run(self, dirs):
        self.window.show_input_panel(
            "新建 APICloud 项目名称:", "", functools.partial(self.on_done, dirs[0]), None, None)

    def on_done(self, dir, name):
        import shutil
        shutil.copytree(os.path.join(curDir, 'appLoader',
                                     'widget', 'default'), os.path.join(dir, name))
        desFile = os.path.join(dir, name, "widget", "config.xml")
...
class NewApicloudBottomAppCommand(sublime_plugin.WindowCommand):
    ...
    def on_done(self, dir, name):
        import shutil
        shutil.copytree(os.path.join(curDir, 'appLoader',
                                     'widget', 'bottom'), os.path.join(dir, name))
        desFile = os.path.join(dir, name, "widget", "config.xml")
        logging.info('desFile: ' + desFile)
        inputFile = open(desFile, encoding='utf-8')
        lines = inputFile.readlines()
        inputFile.close()
        outputFile = open(desFile, 'w', encoding='utf-8')
...
class NewApicloudHomeAppCommand(sublime_plugin.WindowCommand):
    ...
    def on_done(self, dir, name):
        import shutil
        shutil.copytree(os.path.join(curDir, 'appLoader',
                                     'widget', 'home'), os.path.join(dir, name))
        desFile = os.path.join(dir, name, "widget", "config.xml")
        inputFile = open(desFile, encoding='utf-8')
        lines = inputFile.readlines()
        inputFile.close()
...
class NewApicloudSlideAppCommand(sublime_plugin.WindowCommand):
    ...
    def on_done(self, dir, name):
        import shutil
        shutil.copytree(os.path.join(curDir, 'appLoader',
                                     'widget', 'slide'), os.path.join(dir, name))
        desFile = os.path.join(dir, name, "widget", "config.xml")
        inputFile = open(desFile, encoding='utf-8')
        lines = inputFile.readlines()
        inputFile.close()
...
class NewHtml5DefaultAppCommand(sublime_plugin.WindowCommand):
    def run(self, dirs):
        self.window.show_input_panel(
            "新建 HTML5 项目名称:", "", functools.partial(self.on_done, dirs[0]), None, None)

    def on_done(self, dir, name):
        import shutil
        shutil.copytree(os.path.join(curDir, 'appLoader',
                                     'html5', 'default'), os.path.join(dir, name))
...
class CompressWidgetCommand(sublime_plugin.WindowCommand):
    def run(self, dirs):
        ...
        fullzipfilename = os.path.join(
            os.path.dirname(fulldirname), zipfilename)
        logging.info('*' * 30 + 'begin CompressWidgetCommand' + '*' * 30)
        logging.info("CompressWidgetCommand: Begin to zip %s to %s ..." %
                     (fulldirname, fullzipfilename))
        if not os.path.exists(fulldirname):
            logging.info(
                "CompressWidgetCommand: Folder %s is not exist" % fulldirname)
            sublime.error_message(u"文件夹 %s 不存在!" % fulldirname)
            return
        if os.path.exists(fullzipfilename):
            flag = sublime.ok_cancel_dialog(
                u"文件%s 已存在，确定覆盖该文件 ? [Y/N]" % fullzipfilename)
            logging.info(
                "CompressWidgetCommand: %s has already exist" % fullzipfilename)
            if not flag:
